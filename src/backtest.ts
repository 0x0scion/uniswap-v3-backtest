...
export const calcUnboundedFees = (
  globalfee0,
  prevGlobalfee0,
  globalfee1,
  prevGlobalfee1,
  ...
  const fg0_0 =
    parseInt(globalfee0) /
    Math.pow(2, 128) /
    Math.pow(10, poolSelected.token0.decimals);
  const fg0_1 =
  ...
  const fg1_0 =
    parseInt(globalfee1) /
    Math.pow(2, 128) /
    Math.pow(10, poolSelected.token1.decimals);
  const fg1_1 =
...
export const activeLiquidityForCandle = (min, max, low, high) => {
  ...
  const ratioTrue =
    high - low !== 0 ? (Math.min(max, high) - Math.max(min, low)) / divider : 1;
  const ratio = high > min && low < max ? ratioTrue * 100 : 0;

  return isNaN(ratio) || !ratio ? 0 : ratio;
};

// Calculate the number of tokens for a Strategy at a specific amount of liquidity & price
export const tokensFromLiquidity = (
  price,
  low,
  high,
  liquidity,
  ...
  if (sPrice <= sLow) {
    ...
    const amount0 =
      (liquidity * (sPrice - sLow)) / Math.pow(2, 96) / Math.pow(10, decimal1);
    const amount1 =
...
export const liquidityForStrategy = (
  price,
  low,
  high,
  tokens0,
...
export const calcFees = (
  ...
  return data.map((d, i) => {
    const fg =
      i - 1 < 0
        ...
        : calcUnboundedFees(
            ...
            data[i - 1].feeGrowthGlobal0X128,
            d.feeGrowthGlobal1X128,
            data[i - 1].feeGrowthGlobal1X128,
            pool
          );
    ...
    const activeLiquidity = activeLiquidityForCandle(
      minTick,
      maxTick,
      lowTick,
      highTick
    );
    const tokens = tokensFromLiquidity(
      priceToken === 1 ? 1 / d.close : d.close,
      min,
      max,
      liquidity,
    ...
    const tokenRatioFirstClose = tokensFromLiquidity(
      firstClose,
      min,
      max,
      liquidity,
    ...
    if (priceToken === 0) {
      ...
      feeUSD =
        (feeV * parseFloat(latestRec.pool.totalValueLockedUSD)) /
        (parseFloat(latestRec.pool.totalValueLockedToken1) *
          parseFloat(latestRec.close) +
          parseFloat(latestRec.pool.totalValueLockedToken0));
    ...
    return {
      ...
      feeUSD: feeUSD,
      close: d.close,
      baseClose: priceToken === 1 ? 1 / d.close : parseFloat(d.close),
    };
  });
};
...
export const pivotFeeData = (data, priceToken, investment) => {
  ...
  data.forEach((d, i) => {
    if (i > 0) {
      ...
      if (
        currentDate.getUTCDate() === currentPriceTick.day &&
        currentDate.getUTCMonth() === currentPriceTick.month &&
        currentDate.getFullYear() === currentPriceTick.year
      ) {
        ...
        currentPriceTick.fgV = currentPriceTick.fgV + parseFloat(d.fgV);
        currentPriceTick.feeUSD = currentPriceTick.feeUSD + d.feeUSD;
        currentPriceTick.activeliquidity =
          currentPriceTick.activeliquidity + d.activeliquidity;
        currentPriceTick.amountVLast = d.amountV;
        currentPriceTick.count = currentPriceTick.count + 1;

        if (i === data.length - 1) {
          currentPriceTick.activeliquidity =
            currentPriceTick.activeliquidity / currentPriceTick.count;
          currentPriceTick.percFee =
            (currentPriceTick.feeV / currentPriceTick.amountV) * 100;
        }
      } else {
        currentPriceTick.activeliquidity =
          currentPriceTick.activeliquidity / currentPriceTick.count;
