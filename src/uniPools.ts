...
import { minTvl, urlForProtocol, requestBody } from './uniPoolData';
...
export const PoolCurrentPrices = async (signal, protocol, pool) => {
  ...
  try {
    const response = await fetch(
      url,
      requestBody({ query: query, variables: { pool: pool }, signal: signal })
    );
...
export const top50PoolsByTvl = async (signal, protocol) => {
  ...
  const query = `
  query { pools (first:50, where: {totalValueLockedUSD_gt: ${minTvl(
    protocol
  ...
  try {
    const response = await fetch(
      url,
      requestBody({ query: query, signal: signal })
    );
    const data = await response.json();
    // console.log(data)
    if (data && data.data && data.data.pools) {
...
export const poolById = async (id, signal, protocol) => {
  ...
  try {
    const response = await fetch(
      url,
      requestBody({ query: query, variables: { id: id }, signal: signal })
    );
    const data = await response.json();
...
export const poolByIds = async (ids, signal, protocol) => {
  ...
  try {
    const response = await fetch(
      url,
      requestBody({ query: query, variables: { ids: ids }, signal: signal })
    );
    const data = await response.json();
...
export const poolsByTokenId = async (token, signal, protocol) => {
  ...
  const query = `query Pools($token: ID!) {
    pools(where: { token1: $token, totalValueLockedUSD_gt: ${minTvl(
      protocol
    )}} orderBy:totalValueLockedETH, orderDirection:desc ) 
    ${poolQueryFields}
    pools(where: { token0: $token, totalValueLockedUSD_gt: ${minTvl(
      protocol
    )}}, orderBy:totalValueLockedETH, orderDirection:desc ) 
    ${poolQueryFields}
  ...
  try {
    const response = await fetch(
      url,
      requestBody({ query: query, variables: { token: token }, signal: signal })
    );
    const data = await response.json();
...
export const poolsByTokenIds = async (tokens, signal, protocol) => {
  ...
  const query = `query Pools($tokens: [Bytes]!) {
    token1: pools( where: { token1_in: $tokens, totalValueLockedUSD_gt: ${minTvl(
      protocol
    )}}, orderBy:totalValueLockedETH, orderDirection:desc ) 
    ${poolQueryFields}
    token2: pools( where: { token0_in: $tokens, totalValueLockedUSD_gt: ${minTvl(
      protocol
    )}} orderBy:totalValueLockedETH, orderDirection:desc ) 
    ${poolQueryFields}
  }`;
    ...
    if (data && data.data && (data.data.token1 || data.data.token2)) {
      if (data.data.token1 && data.data.token2) {
        const d = data.data.token1.concat(data.data.token2).sort((a, b) => {
          return parseFloat(a.totalValueLockedETH) >
            parseFloat(b.totalValueLockedETH)
            ? -1
            : 1;
        });
